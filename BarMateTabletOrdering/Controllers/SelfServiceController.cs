...
namespace BarMateTabletOrdering.Controllers
{
    ...
    public class SelfServiceController : BaseController
    {
        ...
        [System.Web.Mvc.Authorize(Roles = "SELFSERVICE")]
        public ActionResult Start()
        {
            CabbashViewModel vm = new CabbashViewModel
            {
                PlaceId = 1,
                NameOfPlace = "FARM CITY"
            };
        ...
        public ActionResult GetClickEmoji(string transId)
        {
            return PartialView("_MyEmoji");
        }

        public async Task<ActionResult> GetDeliveryDetails(string tel)
        {
            CabbashViewModel vm = new CabbashViewModel();
            var list = await GetTableItemsByTableTel(tel);
            vm.PurchasedList = list.ToList();
            return PartialView("_ProductOrders", vm);
        }

        [HttpGet]
        public async Task<ActionResult> OpenMyTable()
        {
            ...
            try
            {
                tableNumbers = GetUserById(personId).FirstOrDefault().MiddleName;
            }
            catch
            {
            ...
            else
            {
                ...
                return PartialView("_MySelfTables", m);
            }
        }


        [HttpGet]
        public async Task<ActionResult> OpenMyTableAway()
        {
            int personId = PersonId;
            ...
            else
            {
                ...
                return PartialView("_MySelfTablesAway", m);
            }
        }
        ...
        public async Task<ActionResult> OpenTableByCashier(int? id, bool? isActive, bool? takeaway, string point)
        {
            BarTable bt = null;
            ...
            {
                var A = await GetBarTableByTableId(id.Value);
                bt = A.OrderByDescending(x => x.GuestNumber).FirstOrDefault();
            }
            else
            {
            ...
            if (bt == null)
            {
                ...
                t.StaffId = PersonId;
                t.TableId = id.Value;
                t.TableAlias = "TABLE " + id.Value.ToString() + " - " + "SELF SERVICE";
                t.TableId = id.Value;
                t.TableName = "TABLE " + id.Value.ToString() + " - " + "SELF SERVICE";
                t.GuestNumber = 1;

                if (takeaway.HasValue && takeaway.Value)
                {
                    t.TableAlias = "POINT " + point + " - " + "SELF SERVICE";
                    t.TableId = id.Value;
                    t.TableName = "POINT " + point + " - " + "SELF SERVICE";
                    t.Takeaway = true;
                }
            }
            else
            {
                ...
                t.TableAlias = "TABLE " + bt.TableId.ToString() + " - " + "SELF SERVICE " + (bt.GuestNumber + 1);
                t.TableId = bt.TableId;
                t.TableName = "TABLE " + bt.TableId.ToString() + " - " + "SELF SERVICE " + (bt.GuestNumber + 1);
            ...
            var tof = Request.IsAjaxRequest();


            return View("Index", vm);
        }
        [HttpGet]
        public async Task<ActionResult> OpenTableByCashierXXXXX(int? id, bool? isActive, string telephoneName)
        {
            BarTable bt = null;
            ...
            var loadPosBottom = RenderRazorViewToString("_LoadPOSBottomSS", vm);
        ...
        public async Task<ActionResult> GetMyTables(int? tableId)
        {
            ...
            var t1 = await base.GetBarTableById(tableId.Value);

            vm.TableNumbers = t1.ToList();

            //var ableNumbers = vm.TableNumbers.Where(x => x.TableId == tableId.Value).ToList();
        ...
        public async Task<ActionResult> LoadBottomPos(int? id)
        {
            ...
            return PartialView("_LoadPOSBottomSS", vm);
        }
        ...
        public ActionResult ReProcessTheOrderByTableNumberOne(int? tableId, string kitchenNote, string guestTableNumber)
        {
            ...
            return base.Json(new
            {
                ...
                InsertError = 0,
                LoadPosView = str,
                LoadPosBottom = this.RenderRazorViewToString("_LoadPOSBottomSS", model),
        ...
        public async Task<ActionResult> ReturnChargePaystack(string id, string txnref, string telephone)
        {
            var vm = new CabbashViewModel();
            ...
            int.TryParse(id.Replace("SS", ""), out transId);

            var trans = GetParticularTransactionSingle(transId).FirstOrDefault();

            var kitchenNote = trans.kitchenNote;

            int? tableId = trans.TableId;
            ...
            await UpdateTableWithTelephone(telephone, tableId.Value);

            ProcessSelfServiceToDept(tableId.Value, kitchenNote);

            var ok = await CloseTableAndInsertPayments(tableId.Value, PersonId, trans.Id, trans.Amount);

            var every = ti.ToList();

            vm.ExistingList = every.Where(x => x.TableId == tableId).OrderByDescending(x => x.DateSold).ToList();

            vm.PurchasedList = vm.ExistingList;

            vm.TableId = tableId.Value;

            vm.LoyaltyTelephone = telephone;

            vm.Total = trans.Amount;
        ...
        private int ProcessSelfServiceToDept(int? tableId, string kitchenNote)
        {
            BarTable bt = Enumerable.FirstOrDefault<BarTable>(base.GetBarTableByIdNoAsync(tableId.Value));
        ...
        public async Task<ActionResult> ReturnChargePaystackOld(string id, string txnref)
        {
            var vm = new CabbashViewModel();
            ...
            var btt = await GetBarTableById(tableId.Value);

            BarTable bt = btt.FirstOrDefault();
            ...
            items.ForEach(x => x.DateSold = DateTime.Now);
            items.ForEach(x => x.Cashier = PersonId);
            items.ForEach(x => x.TableId = tableId.Value);
            items.ForEach(x => x.CollectedTime = null);
            items.ForEach(x => x.CompletedTime = null);
            items.ForEach(x => x.Completed = false);
            items.ForEach(x => x.Collected = false);
            items.ForEach(x => x.Fulfilled = false);
            items.ForEach(x => x.IsActive = true);
            items.ForEach(x => x.Note = kitchenNote);


            var kitchenNotes = await InsertBulkItems(items);

            if (kitchenNotes.Any())
            {
                await InsertBulkNotes(kitchenNotes);
            }

            Session[tableCart] = null;

            var lfp = await GetMyTableItemsByTableId(tableId.Value);



            var lstForPrint = lfp.ToList();
            ...
            await CloseTableAndInsertPayments(tableId.Value, PersonId, trans.Id, trans.Amount);
            var ti = await GetMyTableItems(PersonId);
            var every = ti.ToList();
        ...
        private async Task<int> CloseTableAndInsertPayments(int? tableId, int personId, int transactionId, decimal? total)
        {
            ...
            {
                var e = await GetMyTableItems(PersonId);

                var every = e.ToList();

                var tableList = every.Where(x => x.TableId == tableId.Value).OrderByDescending(x => x.DateSold).ToList();

                lst = tableList.Select(x => new POSService.Entities.StockItem { Id = x.StockItemId, Quantity = x.Qty, UnitPrice = x.StockItemPrice, Description = x.StockItemName }).ToList();
            }

            var receiptNumber = DateTime.Now.ToString().GetHashCode().ToString("x");


            StockItemService.UpdateSales(tableId.Value, lst, transactionId, 0, 0, personId, 1, 0, conn, 3, "SELF SERVICE",
                   DateTime.Now, _distributionPointId, true, receiptNumber, total.Value, total.Value, "0%", decimal.Zero, "0%", decimal.Zero, "0%", decimal.Zero,
                   decimal.Zero.ToString(), decimal.Zero, total.Value, decimal.Zero, personId, false);

            return 0;

        }
        public async Task<ActionResult> GetTransactionId(int? placeId, decimal? amount, int? tableId, string kitchenNote)
        {
            var exList = await GetMyTableItemsByTableIdAll(tableId.Value);
            ...
            var transId = CreateTransactionRegistration(tableId.Value, amount.Value, kitchenNote, PersonId);
        ...
        protected int CreateTransactionRegistration(int tableId, decimal foodTotalPrice, string kicthenNote, int staffId)
        {
            int modified = 0;

            if (string.IsNullOrEmpty(kicthenNote))
            {
                kicthenNote = "NONE";
            }
            ...
            {
                using (SqlCommand cmd = new SqlCommand("InsertTransactionPlusKitchen", myConnection))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Amount", foodTotalPrice);
                    cmd.Parameters.AddWithValue("@StaffId", staffId);
                    cmd.Parameters.AddWithValue("@TableId", tableId);
                    cmd.Parameters.AddWithValue("@UniqueId", Guid.NewGuid().ToString());
                    cmd.Parameters.AddWithValue("@kitchenNote", kicthenNote);
